...
In einem ersten Schritt werden die Milchmengendaten für die Monate Januar und Februar 2017 übertragen und verarbeitet. Aufgrund der übertragenen Rohdaten wurden für jede Kuh die Tagesmilchmenge berechnet. Die berechneten Tagesmilchmengen werden als Boxplots dargestellt.
...
tdfResult <- tbl_df(dfResult)
tdfTvdNrCow <- distinct(tdfResult, ATMM_KUH_LIFENR)

### # add month to the data frame
tdfResult <- mutate(tdfResult, ATMM_REC_MONTH = substr(ATMM_REC_DATUM,6,7))

### # function to get TMM per month
tdfGetTmmPerMonth <- function(ptdfResult, psMonth, ptdfTvdNr){
  tdfTmmPerMonth <- filter(ptdfResult, ATMM_REC_MONTH == psMonth)
  ATMM_KUH_IDX <- sapply(tdfTmmPerMonth$ATMM_KUH_LIFENR, 
                         function(x) which(x == ptdfTvdNr$ATMM_KUH_LIFENR), 
                         USE.NAMES = FALSE)
...
sMonthNr <- "01"
tdfResult01 <- tdfGetTmmPerMonth(ptdfResult = tdfResult,
                                 psMonth    = sMonthNr,
                                 ptdfTvdNr  = tdfTvdNrCow)
...
### # produce a vector with TVD-Nr w/out Pz and remove spaces and '.'
vecShortLifeNr <- gsub(" ", "", tdfTvdNrCow$ATMM_KUH_LIFENR, fixed = TRUE)
vecShortLifeNr <- gsub(".", "", vecShortLifeNr, fixed = TRUE)
vecLifeNrTvdNrMap <- sapply(vecShortLifeNr, 
                            function(x) charmatch(x, dfControlMilkYield201701$TVDNR), 
                            USE.NAMES = FALSE)

vecTvdNrLifeNrMap <- sapply(dfControlMilkYield201701$TVDNR,
                            function(x) charmatch(substr(x,1,nchar(x)-1), vecShortLifeNr),
                            USE.NAMES = FALSE)
### # report which cows have milk yield but not TMM data
vecNoTmm <- dfControlMilkYield201701$TVDNR[which(is.na(vecTvdNrLifeNrMap))]
### # bind LifeNrIdx
dfControlMilkYield201701 <- cbind(dfControlMilkYield201701, vecTvdNrLifeNrMap)

### # compute number of total milk yield
tdfControlMilkYield <- tbl_df(dfControlMilkYield201701)
tdfControlMilkYield <- mutate(tdfControlMilkYield, 
                              PESKGLAITTOTAL = PESKGLAITMATIN + PESKGLAITSOIR)

tdfControlMilkYield <- filter(tdfControlMilkYield, !is.na(vecTvdNrLifeNrMap))
...
nNrRowsPerPage <- 37
nNrRounds <- floor(nrow(tdfTvdNrCow)/nNrRowsPerPage)
lstCowNr <- NULL
for (i in 1:nNrRounds){
  lstCurNr <- list(Knr = as.character(c(((i-1)*nNrRowsPerPage + 1):(i*nNrRowsPerPage))),
                   Tnr = as.vector(unlist(tdfTvdNrCow))[((i-1)*nNrRowsPerPage + 1):(i*nNrRowsPerPage)])
...
### # add remainder
if (i*nNrRowsPerPage < nrow(tdfTvdNrCow)){
  vecEmptyRows <- rep("", (nNrRowsPerPage-nrow(tdfTvdNrCow)+i*nNrRowsPerPage))
  lstCurNr <- list(Knr = c(as.character(c((i*nNrRowsPerPage + 1):(nrow(tdfTvdNrCow)))),vecEmptyRows),
                   Tnr = c(as.vector(unlist(tdfTvdNrCow))[(i*nNrRowsPerPage + 1):(nrow(tdfTvdNrCow))],vecEmptyRows))
...
```{r BoxPlotFunction, echo=FALSE, results='hide'}
...
ggp2bpGetBoxPlotAtmm <- function(ptdfResult, psMonthNr, ptdfControlMilkYield){
  ...
  geom_point(aes(vecTvdNrLifeNrMap,PESKGLAITTOTAL), data = ptdfControlMilkYield, color = "red") + 
...
```{r BoxPlotMonth02, echo=FALSE, results='hide', fig.show=TRUE, fig.height=12, eval=FALSE}
### # same for month 2
sMonthNr <- "02"
tdfResult02 <- tdfGetTmmPerMonth(ptdfResult = tdfResult,
                                 psMonth    = sMonthNr,
                                 ptdfTvdNr  = tdfTvdNrCow)
bpAtmmM02 <- ggp2bpGetBoxPlotAtmm(ptdfResult = tdfResult02, psMonthNr = sMonthNr)
bpAtmmM02
```
